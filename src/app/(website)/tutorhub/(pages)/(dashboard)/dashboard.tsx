...
export default function TutorDashboard({ }) {
  ...
  const tutor = useTutorStore((s) => s.data);
  if (!tutor) return <div>Loading…</div>;

  // destructure everything we use
  const {
    user: { name, image },
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">
                    Welcome back, {tutor.user.name?.split(' ')[0] || 'Tutor'}! 👋
                  </h1>
              ...
              <div className="flex items-center space-x-4">
                <div className="text-right">
                  ...
                  <p className="text-2xl font-bold text-green-600 flex items-center">
                    ...
                    {tutor.tutorProfile.hourlyRate || 'Not set'}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          {activeTab === 'overview' && <OverviewTab tutor={ tutor.user } tutorProfile={ tutor.tutorProfile} />}
          {activeTab === 'profile' && <ProfileTab tutor={ tutor.user} tutorProfile={ tutor.tutorProfile} />}
          {activeTab === 'students' && <StudentsTab tutor={ tutor.user } />}
